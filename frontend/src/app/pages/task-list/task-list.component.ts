...
export class TaskListComponent implements OnInit {
  ...
  constructor(
    ...
    this.statusDropdownOptions = this.statusOptions.map(s => ({ label: s.replace('_', ' '), value: s }));
    this.priorityDropdownOptions = this.priorityOptions.map(p => ({ label: p, value: p }));
    this.statusDropdownOptions.unshift({ label: 'Tất cả trạng thái', value: '' });
    this.priorityDropdownOptions.unshift({ label: 'Tất cả độ ưu tiên', value: '' });
  }
  ...
  loadUsers(): void {
    this.userService.getAll().subscribe({ 
      next: (data: User[]) => {
        this.users = data;
        this.userDropdownOptions = data.map(u => ({ label: u.name, value: u.id?.toString() || '' }));
  ...
  openEdit(task: Task): void {
    ...
    // Convert string to Date for DatePicker if needed
    let parsedDueDate: Date | null = null;
    if (task.dueDate) {
      parsedDueDate = new Date(task.dueDate);
    }
  ...
  saveTask(): void {
    ...
    if (formValue.dueDate) {
      if (formValue.dueDate instanceof Date) {
        ...
        const offset = formValue.dueDate.getTimezoneOffset();
        const localDate = new Date(formValue.dueDate.getTime() - (offset * 60 * 1000));
        formattedDueDate = localDate.toISOString().split('T')[0];
      } else {
        formattedDueDate = formValue.dueDate;
      }
    }
