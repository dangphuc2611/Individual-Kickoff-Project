...
@Component({
  selector: 'app-import-an-ninh-mang',
  standalone: true,
      ...
      <div class="max-w-5xl mx-auto px-4 py-8">
        <div class="mb-6 flex items-center justify-between">
            <div>
              ...
              <h1 class="text-2xl font-bold text-slate-800">Nhập dữ liệu Sự cố (Excel)</h1>
            </div>
                      ...
                      <p-table [value]="validRows" [paginator]="true" [rows]="10" styleClass="p-datatable-sm p-datatable-striped">
                          ...
                          <ng-template pTemplate="body" let-row>
                              <tr>
                                  ...
                                  <td>{{ row.rowData.phanLoaiSuCo }}</td>
                                  <td class="font-mono">{{ row.rowData.ipLienQuan }}</td>
...
export class ImportAnNinhMangComponent {
  previewMode = false;
  loading = false;
  saving = false;
  ...
  onFileSelect(event: any) {
    ...
    if (file) {
      ...
      this.service.validateImport(file).subscribe({
        ...
        error: (err) => {
          this.loading = false;
          this.messageService.add({severity:'error', summary:'Lỗi', detail: 'Không thể phân tích file.'});
        }
      });
    }
  }
