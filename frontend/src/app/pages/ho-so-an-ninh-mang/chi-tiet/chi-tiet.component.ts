...
@Component({
  selector: 'app-chi-tiet-an-ninh-mang',
  standalone: true,
  ...
  template: `
    <div class="bg-slate-50 min-h-screen pb-12">
      ...
      <div *ngIf="!loading && detail" class="max-w-5xl mx-auto px-4 py-8">
        ...
        <div class="mb-6">
          ...
          <div>
            <div class="flex gap-2 items-center mb-2">
              <p-tag [value]="detail.loaiTanCong" severity="danger"></p-tag>
        ...
        <p-tabs value="0">
            <p-tablist>
                <p-tab value="0"><i class="pi pi-info-circle mr-2"></i> Thông tin Sự cố</p-tab>
            ...
            <p-tabpanels>
                <p-tabpanel value="0">
                  <div class="bg-white p-6 rounded-xl border border-slate-200 grid grid-cols-2 gap-y-6 gap-x-12">
                    <div>
                      <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-1">Hệ thống bị ảnh hưởng</h3>
                      <p class="text-lg font-medium text-slate-800">{{ detail.heThongBiAnhHuong }}</p>
                    </div>
                    <div>
                      <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-1">Mức độ thiệt hại</h3>
                      <p class="text-lg font-medium text-slate-800">{{ detail.mucDoThietHai || 'N/A' }}</p>
                    </div>
                    <div>
                      <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-1">Ngày phát hiện</h3>
                      <p class="text-slate-800">{{ detail.ngayPhatHien | date:'dd/MM/yyyy' }}</p>
                    </div>
                    <div>
                    ...
                    <div class="col-span-2 mt-4 pt-6 border-t border-slate-100">
                      <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-3">Mô tả chi tiết & Biện pháp</h3>
                      <div class="prose max-w-none text-slate-700 editor-content" [innerHTML]="detail.moTaChiTiet | safeHtml"></div>
                    </div>
                  </div>
                </p-tabpanel>
...
export class ChiTietAnNinhMangComponent implements OnInit {
  id!: number;
  detail: any;
  loading = true;
  auditLogs: any[] = [];
  ...
  ngOnInit() {
    ...
    this.uploadUrl = `/api/ho-so-an-ninh-mang/${this.id}/files`;
    this.fetchDetail();
  }
