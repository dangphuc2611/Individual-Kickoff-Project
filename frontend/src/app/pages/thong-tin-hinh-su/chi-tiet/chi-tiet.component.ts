...
@Component({
  ...
  template: `
    <div class="bg-slate-50 min-h-screen pb-12">
      <div *ngIf="loading" class="flex justify-center items-center h-64"><i class="pi pi-spin pi-spinner text-4xl text-blue-600"></i></div>
      <div *ngIf="!loading && detail" class="max-w-5xl mx-auto px-4 py-8">
        
        <div class="mb-6">
          <button (click)="goBack()" class="text-sm text-slate-500 hover:text-blue-600 mb-4 flex items-center gap-1"><i class="pi pi-arrow-left"></i> Quay lại</button>
          <div>
            <div class="flex gap-2 items-center mb-2">
              <p-tag [value]="detail.loaiToiDanh" severity="danger"></p-tag>
              <p-tag [value]="detail.mucDoMat" severity="warn"></p-tag>
            </div>
            <h1 class="text-3xl font-bold text-slate-800">{{ detail.tieuDe }}</h1>
            <p class="text-slate-500 font-mono mt-2">Mã HS: {{ detail.maThongTin }}</p>
          </div>
        </div>

        <p-tabs value="0">
            <p-tablist>
                <p-tab value="0"><i class="pi pi-info-circle mr-2"></i> Hồ sơ Hình sự</p-tab>
                <p-tab value="1" *ngIf="canViewLogs()"><i class="pi pi-history mr-2"></i> Lịch sử thay đổi</p-tab>
            ...
            <p-tabpanels>
                <p-tabpanel value="0">
                  <div class="bg-white p-6 rounded-xl border border-slate-200 grid grid-cols-2 gap-y-6 gap-x-12">
                    <div>
                      <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-1">Đối tượng / Nghi can</h3>
                      <p class="text-lg font-medium text-slate-800">{{ detail.doiTuongLienQuan }}</p>
                    </div>
                    <div>
                      <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-1">Đơn vị liên quan</h3>
                      <p class="text-lg font-medium text-slate-800">{{ detail.donViLienQuan }}</p>
                    </div>
                    <div>
                      <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-1">Địa điểm xảy ra</h3>
                      <p class="text-slate-800">{{ detail.diaDiem || 'N/A' }}</p>
                    </div>
                    <div>
                      <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-1">Ngày xảy ra</h3>
                      <p class="text-slate-800">{{ detail.ngayXayRa | date:'dd/MM/yyyy' }}</p>
                    </div>
                    <div>
                      <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-1">Kết quả xử lý</h3>
                      <p-tag [value]="detail.ketQuaXuLy" severity="success"></p-tag>
                    </div>
                    
                    <div class="col-span-2 mt-4 pt-6 border-t border-slate-100">
                      <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-3">Mô tả diễn biến</h3>
                      <div class="prose max-w-none text-slate-700 editor-content" [innerHTML]="detail.moTaDienBien | safeHtml"></div>
                    </div>
                  </div>
                </p-tabpanel>
                ...
                <p-tabpanel value="1" *ngIf="canViewLogs()">
                  ...
                  <div class="bg-white p-6 rounded-xl border border-slate-200 min-h-[300px]">
                    ...
                    <div *ngIf="auditLogs.length === 0" class="text-center text-slate-400 mt-10">Chưa có lịch sử.</div>
                  </div>
                </p-tabpanel>
                     ...
                     <div class="mt-8 border-t border-slate-100 pt-6">
                       ...
                       <p-table [value]="files" styleClass="p-datatable-sm">
                          ...
                          <ng-template pTemplate="emptymessage">
                              <tr><td colspan="4" class="text-center p-4">Hồ sơ chưa có tài liệu đính kèm.</td></tr>
                          </ng-template>
                       </p-table>
                     </div>
                   </div>
...
export class ChiTietHinhSuComponent implements OnInit {
  id!: number;
  detail: any;
  loading = true;
  auditLogs: any[] = [];
  ...
  ngOnInit() {
    ...
    this.uploadUrl = `/api/thong-tin-hinh-su/${this.id}/files`;
    this.fetchDetail();
  }
