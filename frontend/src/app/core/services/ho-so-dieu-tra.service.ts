...
import { HoSoDieuTraResponse, PageResponse } from '../models/ho-so.model';

@Injectable({
  providedIn: 'root'
})
export class HoSoDieuTraService {
  private apiUrl = `${environment.apiUrl}/api/ho-so-dieu-tra`;

  constructor(private http: HttpClient) {}

  getAll(page: number, size: number, filters: any = {}): Observable<PageResponse<HoSoDieuTraResponse>> {
    ...
    return this.http.get<PageResponse<HoSoDieuTraResponse>>(this.apiUrl, { params });
  }

  getById(id: number): Observable<HoSoDieuTraResponse> {
    return this.http.get<HoSoDieuTraResponse>(`${this.apiUrl}/${id}`);
  }

  create(hoSo: any): Observable<HoSoDieuTraResponse> {
    return this.http.post<HoSoDieuTraResponse>(this.apiUrl, hoSo);
  }

  update(id: number, hoSo: any): Observable<HoSoDieuTraResponse> {
    return this.http.put<HoSoDieuTraResponse>(`${this.apiUrl}/${id}`, hoSo);
  }
